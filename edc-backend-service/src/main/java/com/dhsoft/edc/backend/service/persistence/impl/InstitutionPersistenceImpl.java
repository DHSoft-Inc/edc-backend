...
public class InstitutionPersistenceImpl
	 ...
	 */
	@Override
	public List<Institution> filterFindByG_P(long groupId, long projectId) {
	 ...
	 */
	@Override
	public List<Institution> filterFindByG_P(
		long groupId, long projectId, int start, int end) {

		return filterFindByG_P(groupId, projectId, start, end, null);
	}
	...
	public List<Institution> filterFindByG_P(
		...
		if (!InlineSQLHelperUtil.isEnabled(groupId)) {
			return findByG_P(groupId, projectId, start, end, orderByComparator);
		}
		...
		if (getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_INSTITUTION_WHERE);
		}
		else {
			sb.append(
				_FILTER_SQL_SELECT_INSTITUTION_NO_INLINE_DISTINCT_WHERE_1);
		}
		...
		if (!getDB().isSupportsInlineDistinct()) {
			sb.append(
				_FILTER_SQL_SELECT_INSTITUTION_NO_INLINE_DISTINCT_WHERE_2);
		}
		...
		if (orderByComparator != null) {
			...
			if (getDB().isSupportsInlineDistinct()) {
				...
				sb.append(InstitutionModelImpl.ORDER_BY_SQL);
			}
		}

		String sql = InlineSQLHelperUtil.replacePermissionCheck(
			sb.toString(), Institution.class.getName(),
		...
		try {
			...
			if (getDB().isSupportsInlineDistinct()) {
				sqlQuery.addEntity(_FILTER_ENTITY_ALIAS, InstitutionImpl.class);
			}
			else {
				sqlQuery.addEntity(_FILTER_ENTITY_TABLE, InstitutionImpl.class);
			}
			...
			return (List<Institution>)QueryUtil.list(
	...
	protected Institution filterGetByG_P_PrevAndNext(
		Session session, Institution institution, long groupId, long projectId,
		OrderByComparator<Institution> orderByComparator, boolean previous) {
		...
		List<Institution> list = sqlQuery.list();
	...
	public int filterCountByG_P(long groupId, long projectId) {
		if (!InlineSQLHelperUtil.isEnabled(groupId)) {
			return countByG_P(groupId, projectId);
		}

		StringBundler sb = new StringBundler(3);

		sb.append(_FILTER_SQL_COUNT_INSTITUTION_WHERE);
	...
	private FinderPath _finderPathFetchByG_P_C;
	...
	public Institution findByG_P_C(long groupId, long projectId, String code)
		...
		Institution institution = fetchByG_P_C(groupId, projectId, code);
	...
	public Institution fetchByG_P_C(long groupId, long projectId, String code) {
		return fetchByG_P_C(groupId, projectId, code, true);
	}
	...
	public Institution fetchByG_P_C(
		...
		if (useFinderCache) {
			finderArgs = new Object[] {groupId, projectId, code};
		}
		...
		if (useFinderCache) {
			result = finderCache.getResult(
				_finderPathFetchByG_P_C, finderArgs, this);
		}
		...
		if (result instanceof Institution) {
			...
			if ((groupId != institution.getGroupId()) ||
				(projectId != institution.getProjectId()) ||
				!Objects.equals(code, institution.getCode())) {
		...
		if (result == null) {
			...
			if (code.isEmpty()) {
				sb.append(_FINDER_COLUMN_G_P_C_CODE_3);
			}
			else {
				bindCode = true;

				sb.append(_FINDER_COLUMN_G_P_C_CODE_2);
			}
			...
			try {
				...
				if (list.isEmpty()) {
					if (useFinderCache) {
						finderCache.putResult(
							_finderPathFetchByG_P_C, finderArgs, list);
					}
				}
				else {
					if (list.size() > 1) {
						...
						if (_log.isWarnEnabled()) {
							...
							_log.warn(
								"InstitutionPersistenceImpl.fetchByG_P_C(long, long, String, boolean) with parameters (" +
				...
				if (useFinderCache) {
					finderCache.removeResult(
						_finderPathFetchByG_P_C, finderArgs);
				}
	...
	public Institution removeByG_P_C(long groupId, long projectId, String code)
		...
		Institution institution = findByG_P_C(groupId, projectId, code);
	...
	public int countByG_P_C(long groupId, long projectId, String code) {
		...
		Object[] finderArgs = new Object[] {groupId, projectId, code};
	...
	private static final String _FINDER_COLUMN_G_P_C_PROJECTID_2 =
		"institution.projectId = ? AND ";

	private static final String _FINDER_COLUMN_G_P_C_CODE_2 =
		"institution.code = ?";

	private static final String _FINDER_COLUMN_G_P_C_CODE_3 =
		"(institution.code IS NULL OR institution.code = '')";
	...
	public void cacheResult(Institution institution) {
		...
		finderCache.putResult(
			_finderPathFetchByG_P_C,
			new Object[] {
				institution.getGroupId(), institution.getProjectId(),
				institution.getCode()
			},
			institution);
	...
	protected void cacheUniqueFindersCache(
		...
		args = new Object[] {
			...
			institutionModelImpl.getProjectId(), institutionModelImpl.getCode()
		};

		finderCache.putResult(
			_finderPathCountByG_P_C, args, Long.valueOf(1), false);
		finderCache.putResult(
			_finderPathFetchByG_P_C, args, institutionModelImpl, false);
	}
	...
	protected void clearUniqueFindersCache(
		...
		if (clearCurrent) {
			Object[] args = new Object[] {
				...
				institutionModelImpl.getCode()
			};

			finderCache.removeResult(_finderPathCountByG_P_C, args);
			finderCache.removeResult(_finderPathFetchByG_P_C, args);
		}

		if ((institutionModelImpl.getColumnBitmask() &
			 _finderPathFetchByG_P_C.getColumnBitmask()) != 0) {

			Object[] args = new Object[] {
				institutionModelImpl.getOriginalGroupId(),
				institutionModelImpl.getOriginalProjectId(),
				institutionModelImpl.getOriginalCode()
			};
	...
	public void activate() {
		...
		_finderPathFetchByG_P_C = new FinderPath(
			entityCacheEnabled, finderCacheEnabled, InstitutionImpl.class,
			FINDER_CLASS_NAME_ENTITY, "fetchByG_P_C",
			new String[] {
				Long.class.getName(), Long.class.getName(),
			...
			InstitutionModelImpl.GROUPID_COLUMN_BITMASK |
			InstitutionModelImpl.PROJECTID_COLUMN_BITMASK |
			InstitutionModelImpl.CODE_COLUMN_BITMASK);
	...
	private static final String _FILTER_SQL_SELECT_INSTITUTION_WHERE =
		"SELECT DISTINCT {institution.*} FROM EDC_Institution institution WHERE ";

	private static final String
		_FILTER_SQL_SELECT_INSTITUTION_NO_INLINE_DISTINCT_WHERE_1 =
			"SELECT {EDC_Institution.*} FROM (SELECT DISTINCT institution.institutionId FROM EDC_Institution institution WHERE ";

	private static final String
		_FILTER_SQL_SELECT_INSTITUTION_NO_INLINE_DISTINCT_WHERE_2 =
			") TEMP_TABLE INNER JOIN EDC_Institution ON TEMP_TABLE.institutionId = EDC_Institution.institutionId";

	private static final String _FILTER_SQL_COUNT_INSTITUTION_WHERE =
		"SELECT COUNT(DISTINCT institution.institutionId) AS COUNT_VALUE FROM EDC_Institution institution WHERE ";

	private static final String _FILTER_ENTITY_ALIAS = "institution";

	private static final String _FILTER_ENTITY_TABLE = "EDC_Institution";

	private static final String _ORDER_BY_ENTITY_ALIAS = "institution.";

	private static final String _ORDER_BY_ENTITY_TABLE = "EDC_Institution.";
